...
| `DanmakuToggle` | 弹幕开关 |
| `DanmakuLayer` | 弹幕渲染层 |
| `DanmakuInputOverlay` | 弹幕发送输入框（需单独导入） |
...
| `setSubtitleWithKey({enabled, trackKey})` | 通过 key 设置字幕 |
| `dispose()` | 释放资源 |
| `state` | 当前播放状态 |
| `qualities` | 可用清晰度列表 |
...
| `videoViewKeySeed` | `Object?` | `null` | 用于强制重建视频视图的 key |
| `danmakuHeightFactor` | `double` | `0.6` | 弹幕显示区域高度比例（0.0-1.0） |
