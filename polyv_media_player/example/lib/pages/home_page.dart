...
import '../player_skin/quality_selector/settings_menu.dart';
import '../player_skin/danmaku/danmaku.dart';
import '../player_skin/danmaku/danmaku_input_overlay.dart';
...
class _LongVideoPageState extends State<LongVideoPage> {
  ...
  Future<void> _initializeServices() async {
    try {
      ...
      debugPrint('LongVideoPage: Failed to load config: $e');
      // 降级到 Mock 服务
  ...
  Future<void> _loadVideoList({bool refresh = false}) async {
    ...
    if (service == null) {
      debugPrint('_loadVideoList: Video list service not initialized yet');
      return;
    }
  ...
  Future<void> _onVideoTap(VideoItem video) async {
    ...
    if (_isSwitchingVideo) {
      debugPrint('_onVideoTap: 视频切换正在进行中，忽略请求');
      return;
    }
    ...
    if (_lastSwitchTime != null) {
      ...
      if (elapsed.inMilliseconds < _debounceMs) {
        debugPrint('_onVideoTap: 防抖中，忽略视频切换请求');
        return;
      }
    }
    ...
    if (_currentVideo?.vid == video.vid) {
      debugPrint('_onVideoTap: 已经在播放视频 ${video.vid}');
      return;
    }
    ...
    try {
      ...
      debugPrint('_onVideoTap: 成功切换到视频：${video.title}');
    } catch (e) {
      debugPrint('_onVideoTap: 视频切换失败：$e');
      ...
      if (mounted && previousVideo != null) {
        ...
        _controller
            ...
            .catchError((error) {
              debugPrint('_onVideoTap: 恢复原视频失败：$error');
            });
