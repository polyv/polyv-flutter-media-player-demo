...
class ActiveDanmaku extends Danmaku {
  ...
  /// 弹幕动画时长（毫秒）
  static const int animationDuration = 8000;
...
class DanmakuSendException implements Exception {
  ...
  static DanmakuSendErrorType _classifyError(Object error) {
    // 根据异常类型或消息内容判断错误类型
    ...
    if (errorStr.contains('network') ||
      ...
      return DanmakuSendErrorType.network;
    }
    ...
    if (errorStr.contains('throttle') ||
      ...
      return DanmakuSendErrorType.throttled;
    }
  ...
  static String _getUserFriendlyMessage(Object error) {
    ...
    if (errorStr.contains('throttle') || errorStr.contains('rate limit')) {
      return '发送过于频繁，请稍后再试';
    }
