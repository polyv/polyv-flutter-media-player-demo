...
class _LongVideoPageState extends State<LongVideoPage> implements DownloadCallbacks {
  ...
  // 视频切换错误恢复：记录切换前的视频
  VideoItem? _previousVideoForRecovery;
  ...
  /// 视频切换：只需改 vid，PolyvVideoPlayer 自动处理遮罩、stop、重建、加载
  void _onVideoTap(VideoItem video) {
    ...
    _previousVideoForRecovery = _currentVideo;
    setState(() {
      _isSwitchingVideo = true;
      _lastSwitchTime = DateTime.now();
      _currentVideo = video;
    });
    // PolyvVideoPlayer 的 didUpdateWidget 检测到 vid 变化后自动：
    // 1. 显示黑色遮罩  2. stop()  3. 重建视频视图  4. loadVideo()
  }

  /// 视频加载成功回调
  void _onVideoLoaded() {
    _previousVideoForRecovery = null;
    if (_isSwitchingVideo && mounted) {
  ...
  void _onVideoError(String error) {
    if (_previousVideoForRecovery != null && mounted) {
      final previous = _previousVideoForRecovery!;
      _previousVideoForRecovery = null;
      _isSwitchingVideo = false;
      setState(() => _currentVideo = previous);
